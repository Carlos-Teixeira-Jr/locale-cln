...
export interface IHomeFilter extends React.ComponentPropsWithoutRef<'div'> {
  ...
  setTransactionType: (_value: TransactionType) => void;
}
...
const HomeFilter: React.FC<IHomeFilter> = ({
  ...
  const buyBtnClassName = `w-[176px] h-9 rounded-full border-black text-quaternary font-bold text-xl transition-all ${
    transactionType === TransactionType.BUY
      ? 'bg-secondary text-quinary border'
      : 'bg-transparent  text-quaternary'
  }`;

  const rentBtnClassName = `w-[176px] h-9 rounded-full border-black text-quaternary font-bold text-xl transition-all ${
  ...
  return (
    <div
      className="flex flex-col md:flex-row gap-4 bg-tertiary px-3 py-6  text-quaternary max-w-[1000px] lg:h-[300px] rounded-[30px] shadow-lg "
      {...homeFilterProps}
    >
      <div className="relative">
        <h1 className="font-bold lg:text-2xl text-xl sm:py-2 lg:pb-4">
          Encontre o lar dos seus sonhos sem sair de casa
        </h1>
        <div className="max-w-[1000px]">
          <label className="text-base sm:py-1">O que procura?</label>

          <div className="flex flex-col md:flex-row lg:flex-row xl:flex-row items-center justify-between gap-5">
            <div className="flex flex-row rounded-full border border-quaternary lg:h-9 w-fit mx-auto">
              <div>
                <button
                  className={buyBtnClassName}
                  onClick={() => handleTransactionType(TransactionType.BUY)}
                >
                  Comprar
                </button>
              </div>
              <div>
                <button
                  className={rentBtnClassName}
                  onClick={() => handleTransactionType(TransactionType.RENT)}
                >
                  Alugar
                </button>
              </div>
            ...
            <div>
              <button
                className="bg-primary rounded-full min-w-[352px] h-10 text-tertiary text-xl shadow-md hover:bg-red-600 hover:text-tertiary hover:shadow-lg transition-all duration-200 active:bg-primary-dark active:text-tertiary active:shadow-none focus:outline-none"
          ...
          <div className="flex flex-col md:flex-row lg:flex-row xl:flex-row justify-between gap-5 items-center">
            <div className="lg:mt-5 md:mt-2 sm:py-1">
              <label>Qual o tipo do imóvel?</label>
              <div
                className="bg-tertiary min-w-[352px] border border-quaternary rounded-lg p-2 flex justify-between"
                ref={ref}
                onClick={() => {
                  setOpen(!open);
                }}
            ...
            <div className="flex flex-col lg:mt-5 md:mt-2">
              ...
              <input
                className="drop-shadow-lg min-w-[352px] bg-transparent border border-quaternary lg:h-11 lg:text-lg rounded-lg p-2 mb-1"
