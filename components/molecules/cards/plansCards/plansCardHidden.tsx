...
const PlansCardsHidden: React.FC<IPlansCardHidden> = ({
  ...
  const dinamicClasses = {
    cardContainer: `md:mx-auto my-5 lg:my-0 rounded-t-[35px] rounded-b-[30px] border-[5px] h-fit ${
      selectedPlanCard === id ? 'border-secondary' : 'border-none'
    }`,
    adminHeader: `${
      isAdminPage ? classes.cardAdminHeader : classes.cardHeader
    }`,
    ...
    contentContainer: `bg-tertiary rounded-b-[30px] p-2 drop-shadow-lg ${
      isAdminPage
    ...
    arrowDown: `float-right cursor-pointer transform transition-transform duration-300 ${
      isAdminPage ? 'mr-5' : 'mr-5'
    } ${selectedCards.includes(id) ? 'rotate-180' : ''}`,
    detailsContainer: `my-1 sm:mx-5 md:mx-0 ${
      isAdminPage
        ? `${selectedCards.includes(id) ? '' : 'hidden'}`
        : `${selectedCards.includes(id) ? '' : 'hidden'}`
    }`,
    locationAndLabel: `${isAdminPage ? 'flex mx-3 mb-5' : 'flex mx-10 mb-7'}`,
    iconLabel: `text-quaternary leading-7 ${
      isAdminPage ? 'font-medium text-base' : 'font-medium text-xl'
    }`,
    highlightContainer: `${isAdminPage ? 'flex mx-3 mb-5' : 'flex mx-10 mb-7'}`,
    p: `text-quaternary leading-7 ${
      isAdminPage ? 'font-medium text-base' : 'font-medium text-xl'
    }`,
    smartAd: `${
      isAdminPage ? 'flex mx-3 mb-5 ' : 'grid grid-flow-col mx-10 mb-7 mt-4'
    }`,
    buttonContainer: `${
      isAdminPage
    ...
    button: `rounded-[10px] text-tertiary mx-auto my-5 font-semibold ${
      isAdminPage
        ? `${
            selectedPlanCard === id
