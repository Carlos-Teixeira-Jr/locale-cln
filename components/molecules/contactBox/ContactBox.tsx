...
const ContactBox: React.FC<IContactBox> = ({ ownerInfo, property }: IContactBox) => {
  ...
  const ownerPropertyWpp = ownerInfo?.wwpNumber ? ownerInfo?.wwpNumber : ownerInfo?.cellPhone;
        ...
        <div className="flex flex-row gap-2 w-full lg:w-72 justify-between items-center md:mt-5">
          {picture ? (
            <Image
              ...
              width={90}
              height={90}
              className="rounded-full w-[90px] max-w-[90px] h-[90px] max-h-[90px] shrink-0 border-2 border-primary object-cover"
            />
          ) : (
            <UserIcon
