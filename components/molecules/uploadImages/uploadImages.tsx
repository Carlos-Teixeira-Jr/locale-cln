...
const UploadImages = ({ 
  ...
  const handleAddImage = (event: any) => {
    ...
    files.forEach((file: any) => {
      // Se vier do banco (url) usar apenas a string se não usar src e id;
  ...
  return (
    <div className="max-w-screen-md mx-auto">
      <label
        className="flex flex-row items-center px-6 w-[255px] h-[50px] border rounded-[50px] bg-secondary cursor-pointer mt-4 ml-2"
      ...
      />
      <p
        className={`text-quaternary font-medium text-xs mt-1 mb-6 ml-2 ${
...
const Image: React.FC<ImageProps> = ({
  ...
  const [, drop] = useDrop({
    accept: 'image',
    hover(
      // item: { type: string | unknown; id: string; index: number },
      item: any,
